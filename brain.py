...
class BettingBrain:
    ...
    def _calculate_momentum(self, stats, minute, dominant):
        try:
            ...
            momentum = (
                min(40, (shots  / minute) * 90 * 2.5) +
                min(35, (sot    / minute) * 90 * 7.0) +
    ...
    def _check_inconsistency(self, stats, dominant):
        try:
            ...
            total_poss = (
                self._safe_get(stats, 'home_poss', 50) +
    ...
    def _analyze_score_context(self, h_s, a_s, dominant, minute, pick):
        try:
            ...
            if minute > 78 and (h_s + a_s) == 0 and 'ÜST' in str(pick):
                issues.append("Geç dakika 0-0'da ÜST riskli")
    ...
    def _phase2_stats_quality(self, stats, minute):
        try:
            ...
            if total_shots >= expected:
                ...
                reasons.append(
                    f"Şut az (beklenen:{int(expected)}, olan:{total_shots})")
    ...
    def _phase3_pressure_trend(self, stats, minute):
        try:
            ...
            # dominant None ise corner_support zaten False kalır
    ...
    def _phase4_value_analysis(self, m, stats, minute,
        ...
        # Varsayılan dönüş değerleri (crash olursa bile bot devam eder)
        ...
        try:
            ...
            dom_xg   = self._calculate_xg(dom['sot'], dom['shots'],
                                           dom['corners'], dom['poss'], minute)
            rec_xg   = self._calculate_xg(rec['sot'], rec['shots'],
                ...
                if minute <= 42:
                    if curr_score == 0 and dom['sot'] >= 2:
                        ...
                        if v >= self.MIN_VALUE_SCORE:
                            picks.append(("İY 0.5 ÜST", 1.60, "Düşük", v))
                else:
                    if curr_score == 0 and dom['sot'] >= 3:
                        ...
                        if v >= self.MIN_VALUE_SCORE:
                            picks.append(("MS 0.5 ÜST", 1.55, "Düşük", v))
                    elif curr_score >= 1 and dom['sot'] >= 2:
                        v = self._calc_value_score(
                            final_p, dom['sot'], dom['shots'],
                            dom['corners'], minute, 'ms_ust_n', dom_xg)
                        if v >= self.MIN_VALUE_SCORE:
                            picks.append((f"MS {curr_score + 0.5} ÜST",
                                          1.55, "Düşük", v))
            ...
            if minute > 52 and total_c >= 8 and corner_support:
                if (total_c / minute) >= 0.13:
                    ...
                    if v >= self.MIN_VALUE_SCORE:
                        picks.append((f"Korner {total_c + 1.5} ÜST",
                                      1.65, "Orta", v))
    ...
    def _calc_confidence(self, final_p, stats, dominant, confirmations, xg):
        try:
            ...
            if prob >= 78:   conf = "🔥 ELITE"
            elif prob >= 70: conf = "⭐ ÇOK YÜKSEK"
            elif prob >= 62: conf = "✅ YÜKSEK"
            else:            conf = "📊 ORTA"
